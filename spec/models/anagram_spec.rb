...
RSpec.describe Anagram do
  ...
  describe '::for' do
    ...
    context 'with one word' do
      ...
      before do
        expect(WordDB).to receive(:anagrams_for).with(word_or_words).and_return(anagrams)
      end
      ...
      context 'with a word containing to alpha numeric charaters' do
        ...
        it 'does not match ' do
          expect(WordDB).to_not receive(:anagrams_for).with('beta')
          expect(described_class.for(word_or_words)).to eq(word_or_words => anagrams)
        end
    ...
    context 'with mulitple comma separated words' do
      ...
      before do
        expect(WordDB).to receive(:anagrams_for).with('beta').and_return(anagrams_for_beta)
        expect(WordDB).to receive(:anagrams_for).with('apt').and_return(anagrams_for_apt)
      end
